...
class NoOpEvaluator(Evaluator):
    def _eval(self, problem, pop, return_values_of, **kwargs):
        ...
        pass
class HA(Algorithm):
    def __init__(self, method="L-BFGS-B", pop_size=100, niche_num=3, mutation_rate=1, inherit_rate=1.0, activate_method=True, X=None, seed=None, **kwargs):
        """
        参数:
    ...
    def evaluate_fitness_cv_batch(self, X):  # X: shape = (N, dim)
        print("进入了evaluate_fitness_cv_batch","X大小",X.shape[0])
        out = {}
        self.problem._evaluate(X, out)
    ...
    def _step_ha(self, pop, fit,cv):
        ...
        # 没有重写的后代
        selected_indices = self._rng.choice(np.setdiff1d(np.arange(self.pop_size), elite_id), offspring_size - num_to_inherit, replace=False)
        selected_offspring = pop[selected_indices]

        elite_individuals = pop[elite_id]
        offspring = np.vstack((offspring, selected_offspring,elite_individuals))
        ...
        if mutate_num > 0:
            mutate_id = self._rng.choice(offspring_size, mutate_num, replace=False)
        ...
        '''
        '''
        print("处理重复个体后 offspring的shape", offspring.shape)
        # 处理重复个体
    ...
    def _clustering_and_learning(self, pop, fit,cv):
        ...
        for i in range(self.niche_num):
            ...
            print("局部优化之前：","x:",pop[best_individual_idx, :],"fitness:",fit[best_individual_idx].item(),"cv:",cv[best_individual_idx].item())
            ...
            print("局部优化之后:", "x:",new_solution[:5], "fitness:",new_fitness.item(),"cv:", new_cv)
            print(f"局部优化消耗了{after-before}次仿真\n")
